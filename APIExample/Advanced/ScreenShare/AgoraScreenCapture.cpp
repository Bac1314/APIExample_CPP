...
bool CAgoraScreenCapture::InitAgora()
{
	...
	//m_lstInfo.InsertString(m_lstInfo.GetCount(), _T("disable local video"));
...
LRESULT CAgoraScreenCapture::OnEIDLocalVideoStateChanged(WPARAM wParam, LPARAM lParam)
{
	...
	case agora::rtc::LOCAL_VIDEO_STREAM_STATE_STOPPED:
		...
		// m_lstInfo.InsertString(m_lstInfo.GetCount(), _T("enableLocalVideo"));
...
void CAgoraScreenCapture::OnBnClickedButtonStartShare()
{
    if (!m_rtcEngine || !m_initialize)
        return;
    HWND hWnd = NULL;
    //if (m_cmbScreenCap.m_listWnd() != m_cmbScreenCap.GetCount() - 1)
