...
struct UploadServerInfo {
  ...
  UploadServerInfo(const char* domain, const char* path, int port, bool https) : serverDomain(domain), serverPath(path), serverPort(port), serverHttps(https) {}
};

struct AdvancedConfigInfo {
  // log upload server

  UploadServerInfo* logUploadServer;

  AdvancedConfigInfo() : logUploadServer(nullptr) {}
};
...
struct LocalAccessPointConfiguration {
  ...
  LocalAccessPointConfiguration() : ipList(nullptr), ipListSize(0), domainList(nullptr), domainListSize(0), verifyDomainName(nullptr), mode(ConnectivityFirst), advancedConfig(nullptr) {}
};
/// @endcond
...
struct ScreenCaptureParameters {
   ...
   * @note If you share a minimized window, you'd better set windowFocus true, or sharing may fail.
   */
  bool windowFocus;
...
struct ScreenCaptureSourceInfo {
  ...
  ScreenCaptureSourceInfo() : type(ScreenCaptureSourceType_Unknown), sourceId(nullptr), sourceName(nullptr), processPath(nullptr), sourceTitle(nullptr), primaryMonitor(false), minimizeWindow(false) {}
#else
